...
export default function PlantDetailPage() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      setPhotoError(t('plants.uploadFailed'))
    } finally {
  ...
  const handleDelete = async () => {
    if (!confirm(t('plants.deleteConfirm', { name: plant.name }))) return
    await deletePlant(id)
    navigate('/')
  }

  if (!plant) return <p className="text-center py-20 text-gray-400">{t('common.loading')}</p>
    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="p-4">
        {editing ? (
          <div className="space-y-3 mb-4">
            <input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" placeholder={t('plants.namePh')} />
            <input value={editForm.species} onChange={e => setEditForm({ ...editForm, species: e.target.value })}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" placeholder={t('plants.speciesLabel')} />
            <input value={editForm.location} onChange={e => setEditForm({ ...editForm, location: e.target.value })}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" placeholder={t('plants.locationLabel')} />
            <textarea value={editForm.notes} onChange={e => setEditForm({ ...editForm, notes: e.target.value })}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" rows={3} placeholder={t('plants.notesLabel')} />
            <div className="flex gap-2">
              <button onClick={handleSave} className="flex-1 bg-green-600 text-white py-2 rounded-lg text-sm font-medium">{t('common.save')}</button>
              <button onClick={() => setEditing(false)} className="flex-1 border border-gray-300 dark:border-gray-600 py-2 rounded-lg text-sm text-gray-700 dark:text-gray-300">{t('common.cancel')}</button>
            </div>
          </div>
        ) : (
          <div className="mb-4">
        ...
        {(() => {
          ...
          return photos.length > 0 ? (
            <div className="mb-4 -mx-4">
              <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide px-4 mb-2">{t('plants.photos')}</h3>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-4">
          {['care', 'journal'].map((tabKey) => (
            <button key={tabKey} onClick={() => { setTab(tabKey); location.hash = tabKey }}
              className={`px-4 py-2 text-sm font-medium capitalize border-b-2 transition-colors ${tab === tabKey ? 'border-green-600 text-green-700 dark:text-green-400' : 'border-transparent text-gray-500 dark:text-gray-400'}`}>
              {t(`plants.${tabKey === 'care' ? 'tabCare' : 'tabJournal'}`)}
            </button>
          ))}
        </div>
                ...
                <div key={s.id} className={`rounded-xl border p-4 ${isDue ? 'border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20' : 'border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <button onClick={() => startEditSchedule(s)} className="text-left">
                        <p className="text-xs text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-colors">
                          {t('plants.every', { count: s.frequency_days })}
                          {s.notify_days_before > 0 ? t('plants.notifyEarly', { count: s.notify_days_before }) : ''}
                        </p>
                      </button>
                      {s.next_due_at && (
                        <p className={`text-xs ${isDue ? 'text-orange-600 dark:text-orange-400 font-medium' : 'text-gray-400 dark:text-gray-500'}`}>
                          {isDue ? t('plants.overdue') : t('plants.due')} {formatDistanceToNow(new Date(s.next_due_at), { addSuffix: true, locale: dateLocale })}
                        </p>
                      )}
                    </div>
                    <button onClick={() => handleMarkDone(s)}
                      ...
                      {justDone[s.id] ? t('common.doneExclaim') : t('common.done')}
                    </button>
                  ...
                  {isEditing && (
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 space-y-2">
                      <div className="flex items-center gap-2">
                        <label className="text-xs text-gray-500 dark:text-gray-400 w-14 shrink-0">{t('plants.everyLabel')}</label>
                        <input
                          type="number" min={1}
                        ...
                        />
                        <span className="text-xs text-gray-500 dark:text-gray-400">{t('plants.days')}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <label className="text-xs text-gray-500 dark:text-gray-400 w-14 shrink-0">{t('plants.notifyLabel')}</label>
                        <select
                        ...
                        >
                          <option value={0}>{t('plants.onDueDay')}</option>
                          {[1,2,3,4,5,6,7].map(d => (
                            <option key={d} value={d}>{t('plants.dayEarly', { count: d })}</option>
                          ))}
                        </select>
                      </div>
                      <div className="flex gap-2 pt-1">
                        <button onClick={() => handleUpdateSchedule(s.id)} className="flex-1 bg-green-600 text-white py-1.5 rounded-lg text-xs font-medium">{t('common.save')}</button>
                        <button onClick={() => setEditingScheduleId(null)} className="flex-1 border border-gray-300 dark:border-gray-600 py-1.5 rounded-lg text-xs text-gray-700 dark:text-gray-300">{t('common.cancel')}</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {showAddSchedule ? (
              <div className="border border-green-200 dark:border-green-800 rounded-xl p-4 space-y-3 bg-green-50 dark:bg-green-900/20">
                ...
                <input
                  ...
                  placeholder={t('plants.customTaskPh')}
                ...
                />
                <div className="flex items-center gap-2">
                  <label className="text-sm text-gray-600 dark:text-gray-300">{t('plants.everyLabel')}</label>
                  ...
                  <label className="text-sm text-gray-600 dark:text-gray-300">{t('plants.days')}</label>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-sm text-gray-600 dark:text-gray-300">{t('plants.notifyLabel')}</label>
                ...
                <div className="flex gap-2">
                  <button onClick={handleAddSchedule} className="flex-1 bg-green-600 text-white py-2 rounded-lg text-sm font-medium">{t('common.add')}</button>
                  <button onClick={() => setShowAddSchedule(false)} className="flex-1 border border-gray-300 dark:border-gray-600 py-2 rounded-lg text-sm text-gray-700 dark:text-gray-300">{t('common.cancel')}</button>
                </div>
              </div>
            ) : (
        ...
        {tab === 'journal' && (
          <div className="space-y-3">
            ...
            {entries.length === 0 && (
              <p className="text-center text-gray-400 py-8 text-sm">{t('plants.noEntries')}</p>
            )}
            {entries.map((entry) => (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-xs text-gray-400 dark:text-gray-500">{format(new Date(entry.entry_date), 'MMM d, yyyy', { locale: dateLocale })}</p>
                      {entry.health && (
                        ...
                        }`}>{t(`common.health.${entry.health}`, { defaultValue: entry.health })}</span>
                      )}
                    </div>
