...
function SchedulePill({ task_type, days_until }) {
  ...
  let label, cls
  if (days_until <= 0) {
    label = days_until < 0 ? t('dashboard.overdue') : t('dashboard.today')
    cls = 'bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400'
  } else if (days_until <= 3) {
    label = t('dashboard.inDays', { count: days_until })
...
function healthLabel(t, health) {
  return t(`common.health.${health}`, { defaultValue: health })
}
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      {notifBanner && (
        <div className="mb-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2 shrink-0">
            ...
            >
              {notifSubscribing ? t('common.enabling') : t('common.enable')}
            </button>
            <button
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{t('dashboard.attention')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1">
            <h3 className="font-semibold text-gray-700 dark:text-gray-300 mb-3">{t('dashboard.dueTasks')}</h3>
            {dueTasks.length === 0 ? (
              <div className="bg-green-50 dark:bg-green-900/30 rounded-2xl p-6 text-center">
                <div className="text-3xl mb-2">✅</div>
                <p className="font-medium text-green-700 dark:text-green-400 text-sm">{t('dashboard.allCaughtUp')}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('dashboard.noTasks')}</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={task.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-800 dark:text-gray-100 text-sm truncate">{plant?.name || t('dashboard.unknown')}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {TASK_ICONS[task.task_type]} {task.task_type} · {formatDistanceToNow(new Date(task.next_due_at), { addSuffix: true, locale: dateLocale })}
                        </p>
                      </div>
                      <button
          ...
          <div className="md:col-span-2">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-semibold text-gray-700 dark:text-gray-300">{t('dashboard.myPlants', { count: plants.length })}</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {plants.map((plant) => (
                ...
                >
                  ...
                  {healthMap[plant.id] && (
                    ...
                    }`}>{healthLabel(t, healthMap[plant.id])}</span>
                  )}
              ...
              >
                ...
                <p className="text-xs mt-1">{t('dashboard.addPlantCard')}</p>
              </Link>
            </div>
          </div>
